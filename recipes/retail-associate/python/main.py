...
# Data from https://www.kaggle.com/datasets/polartech/walmart-grocery-product-dataset
# TODO: impl a sufficient data model
PRODUCT_DB: List[Product] = [
    Product(
        "Tropicana Pure Premium, Original No Pulp 100% Orange Juice, 52 oz Bottle",
        ProductCategory.ORANGE_JUICE,
        3.78
    ),
]
...
class RecipeState(State):
    ...
    def create(
        ...
        if state not in RECIPE_STATE_MAP:
...
class RecipeListenCommandState(State):
    ...
    def run(
        ...
        while True:
            ...
            '''
            findProduct"Where's Great Value orange juice?" → aisle + section + sizes + stock per size
            checkStock"How many Tide pods do we have?" → total + per-size breakdown
            checkPrice"Price check on Equate ibuprofen" → all sizes + prices
            findAssociate"Where's Sarah?" → her current department + status
            messageAssociate"Tell Mike to come to Aisle 12" → simulated message sent
            callForHelp"Need backup at register 5" / "Code three in produce"
            getNextTask"What's my next task?" → next item from task queue
            acknowledge"Roger" / "Copy that" / "On it" — completes a comm
            '''
