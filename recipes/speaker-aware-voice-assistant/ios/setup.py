...
def main() -> None:
    ...
    public_folder = os.path.join(os.path.dirname(__file__), "SpeakerAwareVoiceAssistant", "resources")
    ...
    view_model_path = os.path.join(
        ...
        "SpeakerAwareVoiceAssistant",
        "ViewModel.swift")
    with open(view_model_path, 'r') as view_model_file:
        main_activity_contents = view_model_file.read()
    main_activity_contents = main_activity_contents.replace("${YOUR_ACCESS_KEY_HERE}", args.access_key)
    with open(view_model_path, 'w') as view_model_file:
        view_model_file.write(main_activity_contents)
