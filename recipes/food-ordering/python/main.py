...
class OrderItem:
    @staticmethod
    def from_add_item_inference(inference) -> OrderItem:
        ...
        quantity = inference['slots'].get('quantity', None)

    @staticmethod
    def from_remove_item_inference(inference) -> OrderItem:
        # TODO: this
        size = inference['slots'].get('size', None)
        item = inference['slots'].get('item', None)
        combo = inference['slots'].get('combo', None)
    ...
    @staticmethod
    def from_change_item_inference(inference) -> Tuple[OrderItem, OrderItem]:
        # TODO: this
        size = inference['slots'].get('size', None)
        item = inference['slots'].get('item', None)
        combo = inference['slots'].get('combo', None)
...
class RecipeStandbyState(RecipeState):
    ...
    def run(
        ...
        if len(tasks) == 0:
            return Transition(next_state=RecipeStates.COMPLETE_PROMPT)
...
class RecipeListenForOrderState(RecipePromptState):
    ...
    def run(
        ...
        text = "Listening for order [add, remove, change, start over, help, repeat order, end order]"
        ...
        if inference is not None:
            if inference['is_understood'] and inference['intent'] == 'addItem':
                return Transition(
                    next_state=RecipeStates.ADD_ITEM,
                    next_state_kwargs={
                        'order': order,
                        'item': OrderItem.from_inference(inference)
                    })
                ...
                return Transition(
                    ...
                    next_state_kwargs={
                        'order': order,
                        'item_from': OrderItem.from_inference(inference),
                        'item_to': OrderItem.from_inference(inference)
                    })
                ...
                return Transition(
                    next_state=RecipeStates.SPEECH_RESPONSE,
                    next_state_kwargs={
                        'order': order,
                        'prompt': "Sorry, I didn't understand that. Please try again.",
                    })
